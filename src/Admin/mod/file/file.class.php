...
//        include dirname(__FILE__) . '/tpl/menu.tpl';
    ...
    public function index() {
        ...
        $q = "SELECT * FROM file WHERE rel_name = '" . DB::escape($relName) . "' AND rel_id = $relId";
        $rows = DB::assoc($q);
    ...
    public function upload() {
        ...
        $q = "
            INSERT INTO file SET name = '" . DB::escape($fileInfo['name']) . "', fsname = '" . DB::escape($fsName) . "',
                rel_name = '" . DB::escape($relName) . "', rel_id = $relId
        ";
        $success = DB::query($q);
        if (!$success) {
        ...
        ob_start();
        $q = "SELECT * FROM file WHERE file_id = " . (int) DB::insertID();
        $row = $this->patchFileRow((array) @DB::result($q));
